...
def load_overrides(path: Path | None) -> list[dict[str, str]]:
    ...
    for index, row in enumerate(rows, start=1):
        ...
        normalized = {str(key): str(value).strip() for key, value in row.items() if value is not None}
        missing = [key for key in ("xmltv_id", "site", "site_id") if not normalized.get(key)]
        if missing:
            raise ValueError(f"override #{index} missing required fields: {', '.join(missing)}")
...
def main() -> int:
    ...
    for path in sorted(args.epg_root.glob("sites/**/*.channels.xml")):
        ...
        for element in root.findall("channel"):
            ...
            if site and site_id:
                upstream_entries.setdefault(
                    (site, site_id),
                    {"attrs": attrs, "name": name, "source_file": source_file},
                )
    ...
    for override in overrides:
        ...
        if upstream is None:
            missing_overrides.append(f"{override['xmltv_id']} <- {override['site']}:{override['site_id']}")
            continue
        ...
        for candidate in grouped[xmltv_id]:
            ...
            if signature not in unique:
                ...
                unique[signature]["reasons"] = sorted(
                    set(unique[signature]["reasons"]) | set(candidate["reasons"])
                )
    ...
    print(
        ...
        f"({entries_seen} channel entries scanned, {overrides_loaded} overrides loaded)."
    )
    return 0
