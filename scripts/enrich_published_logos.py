...
def has_valid_icon(channel: ET.Element) -> bool:
    return any((node.attrib.get("src") or "").strip().startswith(("http://", "https://")) for node in channel.findall("icon"))
...
def logo_score(row: dict[str, str]) -> tuple[int, int, int, str]:
    ...
    raster = 1 if fmt in {"PNG", "JPG", "JPEG", "WEBP"} or re.search(r"\.(?:png|jpe?g|webp)(?:\?|$)", url, re.IGNORECASE) else 0
    try:
...
def choose_logo(xmltv_id: str, logos: dict[tuple[str, str], list[dict[str, str]]]) -> str | None:
...
def validate_active_only(root: ET.Element, label: str) -> None:
    ...
    empty = known - active
    if empty:
        raise RuntimeError(f"{label}: empty channels must not be published: {sorted(empty)[:10]}")
...
def channel_map(root: ET.Element) -> dict[str, ET.Element]:
    return {
        (node.attrib.get("id") or "").strip(): node
        for node in root.findall("channel")
        if (node.attrib.get("id") or "").strip()
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Enrich missing logos in published German XMLTV gzip files without changing channel/programme identity.")
